...
All rules apply to every endpoint. Rules are checked in this order:

1. **Request must have `Content-Type: application/json` header** — otherwise return `400` with `{ "error": "Content-Type must be application/json" }`
2. **Body must be valid JSON** — otherwise return `400` with `{ "error": "Invalid JSON body" }`
3. **Both `a` and `b` must be present** — if either is missing return `400` with `{ "error": "Both a and b are required" }`
...
5. **Both `a` and `b` must be within range** — values outside `-1,000,000` to `1,000,000` return `400` with `{ "error": "Values must be between -1000000 and 1000000" }`
